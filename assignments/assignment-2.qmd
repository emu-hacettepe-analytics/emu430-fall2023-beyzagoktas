...
movies_df <- tibble(Title = character(), Year = numeric(), Duration = character(), Rating = numeric(), Votes = numeric())
...
for (url in urls) {
  ...
  temp_df <- tibble(Title = title_names, Year = year, Duration = duration, Rating = rating, Votes = vote)
  
  # Append to the main data frame
  movies_df <- bind_rows(movies_df, temp_df)
}

#The function written above is run to organize the durations of the films
duration_final <- sapply(movies_df$Duration, convert_to_minutes)
movies_df$Duration <- duration_final
